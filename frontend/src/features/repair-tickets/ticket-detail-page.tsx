...
function AttachmentSection({ ticket, attachments, loading, error }: { ticket: RepairTicket; attachments: import("../../types/domain").TicketAttachment[]; loading: boolean; error: unknown }) {
  ...
  return <section aria-labelledby="attachments-title"><div className="section-heading"><div><h2 id="attachments-title">Ảnh đính kèm</h2><p>Chọn ảnh trực tiếp từ thiết bị; hệ thống tự lưu đường dẫn sau khi tải lên.</p></div></div>{loading ? <LoadingState rows={2} /> : error ? <ErrorState error={error} /> : attachments.length === 0 ? <EmptyState title="Chưa có ảnh đính kèm" description="Ảnh trước, trong và sau sửa chữa sẽ xuất hiện tại đây." /> : <div className="attachment-grid">{attachments.map((item) => <a className="attachment-card" href={item.fileUrl} target="_blank" rel="noreferrer" key={item.id}>{item.mimeType?.startsWith("image/") ? <img src={item.fileUrl} alt={item.fileName ?? item.attachmentType} loading="lazy" /> : <span className="attachment-card__fallback" aria-hidden="true">↗</span>}<span className="attachment-card__copy"><strong>{item.fileName ?? item.attachmentType}</strong><small>{item.attachmentType} · {formatDateTime(item.createdAt)}</small></span></a>)}</div>}{types.length > 0 && !terminal ? <form className="attachment-form" onSubmit={(event) => void submit(event)}><FormField label="Mốc ảnh" htmlFor="attachment-type"><select id="attachment-type" value={attachmentType} onChange={(event) => setAttachmentType(event.target.value as TicketAttachmentType)}>{types.map((type) => <option key={type} value={type}>{type}</option>)}</select></FormField><TicketImagePicker file={file} disabled={upload.isPending} onChange={setFile} /><Button type="submit" disabled={!file} loading={upload.isPending}>Tải ảnh lên</Button></form> : null}<MutationError error={upload.error} /></section>;
}
