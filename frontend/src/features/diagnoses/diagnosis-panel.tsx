...
export function DiagnosisPanel({ ticket }: { ticket: RepairTicket }) {
  ...
  return <section className="detail-section" aria-labelledby="diagnosis-title"><div className="section-heading"><div><span className="eyebrow">Kết quả kiểm tra</span><h2 id="diagnosis-title">Chẩn đoán kỹ thuật</h2></div>{latest ? <StatusBadge value={latest.status} /> : null}</div>{!latest && !canCreate ? <EmptyState title="Chưa có chẩn đoán" description={user.role === "CUSTOMER" ? "Kết quả đã được duyệt sẽ xuất hiện tại đây để bạn theo dõi." : "Kỹ thuật viên được phân công sẽ ghi nhận kết quả kiểm tra."} /> : null}{canCreate ? <DiagnosisForm ticketId={ticket.id} /> : null}{latest ? <DiagnosisRecord diagnosis={latest} role={user.role} /> : null}</section>;
}
  ...
  if (editable) return <>
    ...
    {canSubmitDiagnosis(role, diagnosis.status) ? <Card className="diagnosis-card diagnosis-submit-card">
      ...
      <ConfirmDialog
        ...
        description="Bản chẩn đoán đã lưu sẽ tạm thời không thể chỉnh sửa cho đến khi quản lý phản hồi."
...
function CatalogPartSelect({ id, value, options, fallback, onChange }: { id: string; value: number; options: Part[]; fallback?: string; onChange(value: number): void }) {
  ...
  return <select id={id} value={value} onChange={(event) => onChange(Number(event.target.value))}><option value={0}>Chọn linh kiện</option>{value > 0 && !hasCurrent ? <option value={value}>{fallback ?? `Linh kiện #${value}`}</option> : null}{options.map((part) => <option key={part.id} value={part.id}>{part.sku} · {part.name} (còn {part.quantityOnHand})</option>)}</select>;
}
