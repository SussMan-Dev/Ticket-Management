...
- Important columns: unique `ticket_code`, ticket-owned nullable legacy `repair_address`, `status`, `priority`, expectations and milestone timestamps.
- Important indexes: customer, device, status, created time, `(status, priority)`, deleted timestamp.
- Owned by: Repair Tickets.
- Implementation: Phase 4 CRUD/intake is complete; ticket codes use `RT-YYYY-NNNNNN`, customer/device identity is immutable after creation, and new tickets require a repair-address snapshot independent from `customer_profiles.address`. Migration `003_add_repair_ticket_address.sql` keeps legacy rows nullable and authorized staff may correct the address until terminal state.
- Read full schema when: ticket CRUD, filters, transitions, SLA, or timeline changes.

## ticket_attachments

- Purpose: URL metadata for ticket images/proof, including URLs generated by local raw-image storage.
- Primary key: `id`.
...
- Owned by: Repair Tickets.
- Implementation: Phase 4 URL metadata remains supported. JPEG/PNG/WebP files can also be signature/size validated, stored with random names under `uploads/tickets/:ticketId`, and then persisted as generated URL metadata. Role/type checks happen before storage and again under the insert transaction; failed persistence removes the new file. No delete endpoint is exposed.
